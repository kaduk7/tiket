...
function Delete({ mobilId, reload,mobilFoto }: { mobilId: Number, reload: Function ,mobilFoto:String}) {
    ...
    const handleDelete = async (mobilId: number) => {
        setIsLoading(true)
        handleClose()
        await axios.delete(`/admin/api/mobil/${mobilId}`)
        await supabase.storage
        .from(supabaseBUCKET)
        .remove([`foto-mobil/${mobilFoto}`]);
        setTimeout(function () {
            reload()
            setIsLoading(false)
            Swal.fire({
        ...
        <div>
            ...
            <Modal
                ...
                <Modal.Footer>
                    ...
                    <button type="button" className="btn btn-danger light" onClick={() => handleDelete(Number(mobilId))}>Ya, Hapus</button>
                </Modal.Footer>
            </Modal>
        </div>
    )
